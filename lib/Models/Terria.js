...
import Texture from "terriajs-cesium/Source/Renderer/Texture";
import "./i18n.js";
  ...
  return loadJson5(options.configUrl).then(function(config) {
    ...
    return that.serverConfig
      ...
      .then(function() {
        ...
        const disable3D = that.getUserProperty("disable3d");
        if (disable3D) {
          that.viewerMode = ViewerMode.Leaflet;
        } else if (persistViewerMode && defined(that.getLocalProperty("viewermode"))) {
      ...
      .otherwise(function(e) {
        ...
        that.error.raiseEvent({
          ...
          message: `<p>${i18next.t("models.terria.urlLoadErrorMessage")}</p><p>${e.stack || e.message || e.title || ''}</p>`
        });
      });
  });
};

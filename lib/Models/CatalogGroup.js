...
var CatalogGroup = function(terria) {
  ...
  /**
   * Flag for re-requesting data catalog after initial request
   * GeowaveCatalogGroup
   */
  this.isReload = false;
  ...
  knockout.getObservable(this, "isLoading").subscribe(function(newValue) {
    // Console.log("[group.isLoading]", newValue)
...
/**
 * Removes all items or groups from this group.
 */
CatalogGroup.prototype.clear = function() {
  this.items = []
}
...
CatalogGroup.updateItems = function(itemsJson, options, catalogGroup) {
  ...
  for (var itemIndex = 0; itemIndex < itemsJson.length; ++itemIndex) {
    ...
    if(defined(itemObject.name) && itemObject.name === 'GeoWave') {
      itemObject.items.forEach(item=> item.isEnabled = false)
    }
    ...
    if(options.isUserSupplied) {
      // 데이터 카탈로그가 변경되면 요청한 시간을 현 시간으로 초기화 해주자...
      itemObject.requestTime = new Date().getTime() - 60000
      if(itemObject.isOpen) itemObject.isOpen = false
    }

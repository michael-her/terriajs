...
export const TWKB_VISUAL_CONFIGS = {
  opacity: 'opacity',
  strokeOpacity: {
...
var GeowaveTwkbCatalogItem = function(terria, url) {
  ...
  this.extrudedHeight = new CallbackProperty((time, result) => this.visual.extruded ? this.visual.extrudedHeight : undefined, true)
  ...
  overrideProperty(this, 'initial', {
    get: function() {
      ...
      return Object.keys(TWKB_VISUAL_CONFIGS).reduce((ret, item) => {
        if (!defined(ret[item]) && LAYER_VIS_CONFIGS[TWKB_VISUAL_CONFIGS[item]]) {
          // if assigned one of default LAYER_CONFIGS
          ret[item] = LAYER_VIS_CONFIGS[TWKB_VISUAL_CONFIGS[item]].defaultValue;
        } else if (['type', 'defaultValue'].every(p => TWKB_VISUAL_CONFIGS[item].hasOwnProperty(p))) {
          // if provided customized visConfig, and has type && defaultValue
          // TODO: further check if customized visConfig is valid
          ret[item] = defaultValue(ret[item], TWKB_VISUAL_CONFIGS[item].defaultValue);
        }
        return ret
      }, INITIAL)
    },
  ...
  overrideProperty(this, "setting", {
    get: function() {
      return Object.keys(TWKB_VISUAL_CONFIGS).reduce((ret, item) => {
        if (LAYER_VIS_CONFIGS[TWKB_VISUAL_CONFIGS[item]]) {
          if (defined(this.initial[item])) {
            ret[item] = update(LAYER_VIS_CONFIGS[TWKB_VISUAL_CONFIGS[item]], {defaultValue: {$set: this.initial[item]}})
          } else {
            // if assigned one of default LAYER_CONFIGS
            ret[item] = LAYER_VIS_CONFIGS[TWKB_VISUAL_CONFIGS[item]];
          }
        } else if (['type', 'defaultValue'].every(p => TWKB_VISUAL_CONFIGS[item].hasOwnProperty(p))) {
          // if provided customized visual, and has type && defaultValue
          // TODO: further check if customized visual is valid
          ret[item] = TWKB_VISUAL_CONFIGS[item];
        }
        return ret
      }, {
        colorUI: {
          ...
          return jsonPromise
            .then(function(response) {
              ...
              if (JSON.stringify(fields) !== JSON.stringify(that.config.fields) || geomType !== that.config.geomType) {
                ...
                const visual = _.pick(that.initial, ['colorField', 'strokeColorField', 'sizeField', 'colorDomain', 'strokeColorDomain', 'sizeDomain', 'extrudedHeightDomain'])
 ...
 */
 GeowaveTwkbCatalogItem.prototype.updateLayerVisual = function(layer, props) {
  ...
  if (_.has(props, 'color') || _.has(props, 'filled')) {
    this.color.setCallback((time, result) => fromArray(this.visual.color, this.visual.opacity, result), true)
  }
  if (_.has(props, 'strokeColor') || _.has(props, 'stroked')) {
    this.strokeColor.setCallback((time, result) => fromArray(this.visual.strokeColor, this.visual.opacity, result), true)
  }
  if (_.has(props, 'thickness') || _.has(props, 'stroked')) {
    this.thickness.setCallback((time, result) => this.visual.thickness, true)
  }
  if (_.has(props, 'size')) {
    this.size.setCallback((time, result) => this.visual.size, true)
  }
  if (_.has(props, 'radius')) {
    this.radius.setCallback((time, result) => this.visual.radius, true)
  }
  if (_.has(props, 'extrudedHeight') || _.has(props, 'extruded')) {
    this.extrudedHeight.setCallback((time, result) => this.visual.extruded ? this.visual.extrudedHeight : undefined, true)
  }

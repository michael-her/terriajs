...
var GeowaveTwkbCatalogItem = function(terria, url) {
  ...
  /**
    ...
    */
  this.fillColor = new CallbackProperty((time, result) => fromArray(this.visual.color, result), false)
   ...
   * @default '#519AC2'
   */
  this.lineColor = new CallbackProperty((time, result) => fromArray(this.visual.strokeColor, result), false)
  ...
  /**
    ...
    * @default 1.0
    */
  this.lineWidth = new CallbackProperty((time, result) => this.visual.thickness, true)
          ...
          return jsonPromise
            .then(function(response) {
              ...
              const features = json.data.rows.map(row => {
                ...
                // const bstr = window.atob(row.shape)
              ...
              that.config.geomType = _.get(features, '[0].geometry.type')
              that.config.fields = json.data.columnsWithSchema
 ...
 */
 GeowaveTwkbCatalogItem.prototype.updateLayerVisual = function(layer, props) {
  ...
  if (_.has(props, 'filled') || _.has(props, 'stroked')) {
...
GeowaveTwkbCatalogItem.prototype.updateLayerChannel = function(layer, props, channel, scaleKey) {
...
GeowaveTwkbCatalogItem.prototype.getScaleOptions = function(ch) {
  const {field, scale, channelScaleType} = this.channels[ch];
  ...
  return dataSource.load(item._readyData, options).then(function() {
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      if (defined(entity.polygon)) {
        ...
        entity.polygon.outlineWidth = item.lineWidth // overwrite
        ...
        if (item.visual.stroked && !item.visual.filled) {
          ...
          if (defined(entity.polygon.outlineColor)) {
            ...
            entity.polyline.material.color = item.lineColor
          }

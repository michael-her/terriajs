...
import { JSON, Object } from 'global'
...
var GeowaveTwkbCatalogItem = function(terria, url) {
  ...
  /**
    ...
    */
  this.color = new CallbackProperty((time, result) => fromArray(this.visual.color, result), false)
   ...
   * @default '#519AC2'
   */
  this.strokeColor = new CallbackProperty((time, result) => fromArray(this.visual.strokeColor, result), false)
      ...
      promiseFunctionToExplicitDeferred(codeSplitDeferred, function() {
        ...
        if (defined(that.data)) {
          ...
          if (zipFileRegex.test(that.url)) {
            ...
            jsonPromise = loadTwkb(proxyCatalogItemUrl(that, that.url, "1d"), JSON.stringify(that.urlAttachment), that.token);
          }
          ...
          return jsonPromise
            .then(function(response) {
              ...
              const features = json.data.rows.map(row => {
                const {shape, ...properties} = row
              ...
              .map(field =>
                Object.keys(field).reduce((ret, key) => ({
                  ...ret,
                  [key === 'type' ? 'localType' : key]: field[key]
                }), {
                  valueAccessor(d) { return d.properties[field.name].getValue() }
                })  
              )
              const geomType = _.get(features, '[0].geometry.type')
              ...
              if (JSON.stringify(fields) !== JSON.stringify(that.config.fields) || geomType !== that.config.geomType) {
                ...
                that.color.setCallback((time, result) => fromArray(that.visual.color, result), false)
                that.strokeColor.setCallback((time, result) => fromArray(that.visual.strokeColor, result), false)
              ...
              // Console.log('[item._load]', data)
              return updateModelFromData(that, data);
            })
            .otherwise(function(e) {
              ...
              throw new TerriaError({
                ...
                message: e.message,
                // i18next.t("models.gwTwkb.couldNotLoadMessage", {
                //   cors:
...
GeowaveTwkbCatalogItem.prototype.repaint = debounce(function() {
...
GeowaveTwkbCatalogItem.prototype.updateLayerChannel = function(layer, props, channel) {
  ...
  ['color', 'strokeColor', 'size', 'radius'].forEach(key => {
    if (props[`${key}Field`]) {
      this.visual[`${key}Domain`] = this.styleCache.table.getColumnFilterDomain(this.visual[`${key}Field`]).domain // 캐시 대상
      Console.log('[item.updateLayerDomain]', this.visual[`${key}Domain`])
      this.styleCache.accessors[key] = getAttributeAccessor(
        this.channels[key],
        this.visual[`${key}Field`],
        this.visual[`${key}Scale`],
      ...
      this[key].setCallback((time, result, entity) => {
        return fromArray(this.styleCache.accessors[key](entity), result)
      }, false)
      this.terria.store.dispatch(layerVisualChange(this.uniqueId, {[`${key}Domain`]: this.visual[`${key}Domain`]}))
    } else {
      this[key].setCallback((time, result) => fromArray(this.visual[key], result), false)
    }
  });

  // Scale
  ['color', 'strokeColor', 'size', 'radius'].filter(
    key => props[`${key}Scale`] && layer.visual[`${key}Field`]
  ).forEach(key => {
    this.styleCache.accessors[key] = getAttributeAccessor(
      this.channels[key],
      this.visual[`${key}Field`],
  ...
  //   if (props.colorField) {
  //     this.visual.colorDomain = this.styleCache.table.getColumnFilterDomain(this.visual.colorField).domain // 캐시 대상
  //     Console.log('[twkb.updateLayerDomain]', this.visual.colorDomain)
  //     this.styleCache.accessors.color = getAttributeAccessor(
  //       this.channels.color,
  //       this.visual.colorField,
  //       this.visual.colorScale,
  //       this.visual.colorDomain,
  //       this.visual.colorRange,
  //       d => d
  //     )
  //     this.color.setCallback((time, result, entity) => {
  //       return fromArray(this.styleCache.accessors.color(entity), result)
  //     }, false)
  //     this.terria.store.dispatch(layerVisualChange(this.uniqueId, {colorDomain: this.visual.colorDomain}))
  //   } else {
  //     this.color.setCallback((time, result) => fromArray(this.visual.color, result), false)
  //   }
  // }

  // if (_.has(props, 'strokeColorField')) {
  //   if (props.strokeColorField) {
  //     this.visual.strokeColorDomain = this.styleCache.table.getColumnFilterDomain(this.visual.strokeColorField).domain // 캐시 대상
  //     Console.log('[twkb.updateLayerDomain]', this.visual.strokeColorDomain)
  //     this.styleCache.accessors.strokeColor = getAttributeAccessor(
  //       this.channels.strokeColor,
  //       this.visual.strokeColorField,
  //       this.visual.strokeColorScale,
  //       this.visual.strokeColorDomain,
  //       this.visual.strokeColorRange,
  //       d => d
  //     )
  //     this.lineColor.setCallback((time, result, entity) => {
  //       return fromArray(this.styleCache.accessors.strokeColor(entity), result)
  //     }, false)
  //   } else {
  //     this.lineColor.setCallback((time, result) => fromArray(this.visual.strokeColor, result), false)
  //   }
  // }
...
function loadGeoJson(item) {
  ...
  //   markerColor: item.fillColor,
  //   strokeWidth: 2, // item.lineWidth,
  //   polygonStroke: item.lineColor,
  //   polylineStroke: item.lineColor,
  ...
  //   markerUrl: defaultValue(style["marker-url"], null), // not in SimpleStyle spec but gives an alternate to make marker symbols
  //   fill: item.fillColor,
  // };
  ...
  return dataSource.load(item._readyData, options).then(function() {
    ...
    .reduce((ret, key) => ({
      ...
      [key]: getAttributeAccessor(
        ...
        item.styleCache.table.getColumnFilterDomain(item.visual[`${key}Field`]).domain, // 실질적인 캐싱 대상
...
function fromArray([r, g, b, a = 255], result) {
  if (!defined(result)) {
    result = new Color(r / 255.0, g / 255.0, b / 255.0, a / 255.0)
  } else {
    result.red = r / 255.0
    result.green = g / 255.0
    result.blue = b / 255.0
    result.alpha = a / 255.0
  }
  return result
}

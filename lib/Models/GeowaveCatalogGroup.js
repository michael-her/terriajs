...
GeowaveCatalogGroup.prototype._load = function() {
  var url = cleanAndProxyUrl(this, this.url) + "api/layers";
...
function createGeowaveDataSource(
  ...
  //TODO
  var result = new MapboxVectorTileCatalogItem(thisGroup.terria);

  result.name = getNameFromLayer(thisGroup, layer);
  result.layer = layer.name; // layer name
  result.url = `http://192.168.0.221:18080/geoserver/gwc/service/tms/1.0.0/geowave:${result.layer}@EPSG:900913@pbf/{z}/{x}/{-y}.pbf`
  result.metadataUrl = `http://192.168.0.221:18080/geoserver/wfs?service=wfs&version=2.0.0&request=DescribeFeatureType&typeName=geowave:${result.layer}&outputFormat=application/json`
  result.maximumNativeZoom = 23
  result.treat404AsError = false

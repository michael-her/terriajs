...
function cleanAndProxyUrl(catalogGroup, url) {
  ...
  return proxyCatalogItemUrl(catalogGroup, cleanedUrl, "0s");
...
function addLayersRecursively(
  ...
  if (parentGroup.isReload) {
    for (const item of parentGroup.items) {
      const idx = layers.findIndex(layer=>layer.name===item.name)
      if(idx===-1) {
    ...
    for (const layer of layers) {
      const idx = parentGroup.items.findIndex(item => item.name === layer.name)
      if(idx===-1) {
        parentGroup.add(

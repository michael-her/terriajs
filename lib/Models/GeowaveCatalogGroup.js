...
function createGeowaveDataSource(
  ...
  result.url = `${serverURI}/gwc/service/tms/1.0.0/${layer.viewName}@MVT_EPSG:900913@pbf/{z}/{x}/{-y}.pbf`
  if (serverURIs.length > 1) {
    result.urls = serverURIs.map(uri => `${uri}/gwc/service/tms/1.0.0/${layer.viewName}@MVT_EPSG:900913@pbf/{z}/{x}/{-y}.pbf`)
  }

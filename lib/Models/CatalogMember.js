...
CatalogMember.prototype.load = function() {
  ...
  if (arraysAreEqual(loadInfluencingValues, this._lastLoadInfluencingValues)) {
    //TODO requestTime 검증 필요함 : 그룹 클릭 시 첫 요청시간과 1분이 지나면 재요청을 위한 로직.. 검증 필요
    if(defined(this.requestTime) && (new Date().getTime() - this.requestTime)/1000/60 > 1){
      this.isReload = true
    }// this.clear()
    else// Already loaded, and nothing has changed to force a re-load.
    return undefined;
  }

...
import { Array, Date, Map, Object } from 'global';
...
import { defaultGetFieldValue } from '../Layers/base-layer'
...
var CatalogItem = function(terria) {
  ...
  /**
   * Gets or sets a value indicating whether this data item is configurable.
   * @type {Boolean}
   */
  ...
  knockout.getObservable(this, "isConfigurable").subscribe(function(newValue) {
    isConfigurableChanged(this);
  }, this);
  ...
  overrideProperty(this, 'visual', {
    get: function() {
      return Object.keys(baseVisual).reduce((ret, item) => {
        if (typeof item === 'string' && LAYER_VIS_CONFIGS[baseVisual[item]]) {
          if (defined(this.initial[item])) {
            ret[item] = this.initial[item]
          } else {
            // if assigned one of default LAYER_CONFIGS
            ret[item] = LAYER_VIS_CONFIGS[baseVisual[item]].defaultValue;
          }
        } else if (['type', 'defaultValue'].every(p => baseVisual[item].hasOwnProperty(p))) {
          // if provided customized visConfig, and has type && defaultValue
          // TODO: further check if customized visConfig is valid
          ret[item] = baseVisual[item].defaultValue;
        }
        return ret
      }, {})
    },
  ...
  overrideProperty(this, "setting", {
    get: function() {
      return Object.keys(baseVisual).reduce((ret, item) => {
        if (typeof item === 'string' && LAYER_VIS_CONFIGS[baseVisual[item]]) {
          if (defined(this.initial[item])) {
            ret[item] = update(LAYER_VIS_CONFIGS[baseVisual[item]], {defaultValue: {$set: this.initial[item]}})
          } else {
            // if assigned one of default LAYER_CONFIGS
            ret[item] = LAYER_VIS_CONFIGS[baseVisual[item]];
          }
        } else if (['type', 'defaultValue'].every(p => baseVisual[item].hasOwnProperty(p))) {
          // if provided customized visual, and has type && defaultValue
          // TODO: further check if customized visual is valid
          ret[item] = baseVisual[item];
        }
        return ret
      }, {})
    },
...
// overrideProperty(this, "setting", {
//   get: function() {
//     return Object.keys(baseVisual).reduce((setting, item) => {
//       if (typeof item === 'string' && LAYER_VIS_CONFIGS[baseVisual[item]]) {
//         if (defined(this.initial[item])) {
//           setting[item] = update(LAYER_VIS_CONFIGS[baseVisual[item]], {defaultValue: {$set: this.initial[item]}})
//         } else {
//           // if assigned one of default LAYER_CONFIGS
//           setting[item] = LAYER_VIS_CONFIGS[baseVisual[item]];
//         }
//       } else if (['type', 'defaultValue'].every(p => mvtVisual[item].hasOwnProperty(p))) {
//         // if provided customized visual, and has type && defaultValue
//         // TODO: further check if customized visual is valid
//         setting[item] = baseVisual[item];
//       }
//       return setting
//     }, {
//       colorUI: {
...
Object.defineProperties(CatalogItem.prototype, {
  ...
  /**
   * Gets a value indicating whether the configuration of this data item can be toggled.
   * @memberOf CatalogItem.prototype
   * @type {Boolean}
   */
  supportsToggleConfigurable: {
    get: function() {
      return true;
    }
  },
...
CatalogItem.defaultPropertiesForSharing.push("isConfigurable");
...
/**
 * methods from kepler.gl base-layer
 */
CatalogItem.prototype.updateLayerVisual = function(layer, props) {
}

CatalogItem.prototype.isLayerHovered = function(objectInfo) {
  return objectInfo?.picked && objectInfo?.layer?.props?.id === this.layer;
}

CatalogItem.prototype.shouldCalculateLayerData = function(props) {
  return props.some(p => !this.noneLayerDataAffectingProps.includes(p));
}

CatalogItem.prototype.shouldRenderLayer = function(data) {
  return true
 ...
 */
CatalogItem.prototype.getVisualChannelDescription = function(key) {
  // e.g. label: Color, measure: Vehicle Type
  return {
    label: this.setting[this.channels[key].range].label,
    measure: this.visual[this.channels[key].field]
      ? this.visual[this.channels[key].field].name
      : this.channels[key].defaultMeasure
  };
}

CatalogItem.prototype.isValidToSave = function() {
 ...
 */
CatalogItem.prototype.hasAllColumns = function() { return false }

CatalogItem.prototype.getColorScale = function(colorScale, colorDomain, colorRange) {
  ...
  return this.getChannelScale(colorScale, colorDomain, colorRange.colors.map(hexToRgb))
}

CatalogItem.prototype.getChannelScale = function(scale, domain, range, fixed) {
  return SCALE_FUNC[fixed ? 'linear' : scale]()
    .domain(domain)
    .range(fixed ? domain : range)
}

CatalogItem.prototype.getEncodedChannelValue = function(
...
/**
 * Toggles the {@link CatalogItem#isConfigurable} property of this item. If it is configurable, calling this method
 * will unable it. If it is unable, calling this method will configurable it.
 * 
 * @returns {Boolean} true if the item is now configurable, false if it is now unable
 */
CatalogItem.prototype.toggleConfigurable = function() {
  this.isConfigurable = !this.isConfigurable;
  // console.log('toggleConfigurable:', this.isConfigurable);
...
CatalogItem.prototype.toggleLegendVisible = function() {
  ...
  // console.log('toggleLegendVisible:', this.isLegendVisible);
...
CatalogItem.prototype.toJS = function() {
  // TODO: 이렇게 정제할 것이 아니라 딱 필요한 프로퍼티만 _.pick 해서 복사하도록 하자.
  const refined = update(this, {$unset: [
    // GeowaveVectorTileCatalogItem
    'data',
    'tileCache',
    'styleCache',
    'parent',
    ...
    '_dataSource',
    '_tableStructure',
    '_readyData', // from GeoJsonCatalogItem
    'tableStructure',
    'metadata',
    ...Object.keys(this).filter(key => typeof this[key] === 'object' && this[key] instanceof CallbackProperty)
  ]})
  Console.log('[item.toJS]', {refined})
  const clone = cloneDeep(refined);
  ...
  // replace ko.Observable to js
  Object.keys(clone).filter(key => knockout.isObservable(clone[key])).forEach(key => {
    const value = knockout.toJS(clone[key])
    delete clone[key]
    clone[key] = value
  })

  clone.id = clone.id || knockout.toJS(this.uniqueId)
  ...
  //   '_imageryLayer',
  //   '__knockoutObservables'
  // ].includes(key)).reduce((js, key) => ({
  //   ...js,
  //   [key]: knockout.isObservable(this[key]) ? knockout.toJS(this[key]) : this[key]
  // }), {
  //   type: this.type,
  ...
  if (catalogItem.isEnabled) {
    ...
    if (!defined(catalogItem._loadForEnablePromise)) {
      ...
      var loadPromise = when
        ...
        .then(function() {
          if (catalogItem.isEnabled) {
            // michael: item.load() 가 끝난다음에 UI에 추가하도록 하자
            // 상태관리를 할 필요가 없는 Terria 인스턴스는 store를 가지지 않는다.
            terria.store && terria.store.dispatch(addLayer({item: catalogItem.toJS()}))
    ...
    // michael
    // 상태관리를 할 필요가 없는 Terria 인스턴스는 store를 가지지 않는다.
    terria.store && terria.store.dispatch(removeLayer(catalogItem.uniqueId))
...
function isConfigurableChanged(catalogItem) {
  catalogItem.terria.currentViewer.notifyRepaintRequired();
}

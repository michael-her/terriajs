...
GeowaveVectorTileCatalogItem.prototype.loadLayerMetadata = function() {
  ...
  return resource.fetch().then(response => {
    ...
    this._previewInfo = {
      epsg: _.get(rawLayerMetadata, 'layer.spatialInfo.epsg'),
      envelope: _.get(rawLayerMetadata, 'layer.spatialInfo.envelope'),
    }
...
GeowaveVectorTileCatalogItem.prototype._remove = function (){
  Console.log('[itme._remove]',this)
  const that = this;
  const url = `${that.parent.url}/api/mapviews/${that.nameInCatalog}?all=true`
  Resource.delete(proxyCatalogItemUrl(that, url, '0s')).then((r)=>{

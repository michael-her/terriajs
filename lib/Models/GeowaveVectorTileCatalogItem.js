...
const GeowaveVectorTileCatalogItem = function(terria) {
  ...
  overrideProperty(this, "config", {
    get: function() {
      let geomField = undefined
      let geomType = undefined
...
GeowaveVectorTileCatalogItem.prototype.requestColumnsMetadata = function() {
  if (!defined(this.metadataUrl)) {
    return;
  }
  const resource = new Resource({url: this.metadataUrl});
  return resource.fetch().then(response => {
    this.metadata.columnMetadata = JSON.parse(response);
    if (this.metadata.columnMetadata) {
      this.config.namespace = _.get(this.metadata.columnMetadata, 'targetNamespace', undefined)
      this.config.dataId = _.get(this.metadata.columnMetadata, 'featureTypes[0].typeName', undefined)
      this.config.fields = _.get(this.metadata.columnMetadata, 'featureTypes[0].properties', [])
      ...
      Console.log('[item.requestColumnsMetadata]', {metadata: this.metadata.columnMetadata, config: this.config})
    } else {
...
GeowaveVectorTileCatalogItem.prototype._load = function() {
  ...
  return promise.then(function(token) {
    ...
    let promises = [Promise.resolve(that.requestColumnsMetadata())]
...
GeowaveVectorTileCatalogItem.prototype.calculateLayerDomain = function(table, channel) {
  const {scale} = channel;
  const scaleType = this.visual[scale];

  const field = this.visual[channel.field];
  if (!field) {
 ...
 */
GeowaveVectorTileCatalogItem.prototype.updateLayerDomain = function(newFilter) {
  this.styleCache.table && Object.values(this.channels).forEach(channel => {
    const {scale} = channel;
    const scaleType = this.visual[scale];
    ...
    if (!newFilter || scaleType !== SCALE_TYPES.ordinal) {
      ...
      const updatedDomain = this.calculateLayerDomain(this.styleCache.table, channel);
      this.updateLayerVisual({[domain]: updatedDomain});
    }
  })
}

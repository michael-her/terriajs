...
MapboxVectorTileCatalogItem.prototype.requestMetadata = function() {
  ...
  return resource.fetch().then(response => {
    ...
    if (this.metadata) {
      ...
      console.log('_load', this.metadata)
    } else {
...
MapboxVectorTileCatalogItem.prototype._load = function() {
  console.log('MapboxVectorTileCatalogItem.prototype._load')
...
MapboxVectorTileCatalogItem.prototype.updateLayerVisual = function(layer, props) {
  ...
  if (_.has(props, 'opacity')) {
    this.opacity = _.get(props, 'opacity')
  }
  if (_.has(props, 'color')) {
    this.fillColor = rgba.css(_.get(props, 'color'))
  }
  if (_.has(props, 'colorRange')) {
    // js 디스트럭팅을 쓰는것보다 오류체크가 쉬움
    this.fillColor = update(this.fillColor, {range: {$set: _.get(props, 'colorRange')}})
  }
  if (_.has(props, 'strokeColor')) {
    this.lineColor = rgba.css(_.get(props, 'strokeColor'))
  }
  if (_.has(props, 'strokeColorRange')) {
    // js 디스트럭팅을 쓰는것보다 오류체크가 쉬움
    this.lineColor = update(this.lineColor, {range: {$set: _.get(props, 'strokeColorRange')}})
  }
  if (_.has(props, 'thickness')) {
    this.lineWidth = _.get(props, 'thickness')
  }
  if (_.has(props, 'filled')) {
    const color = layer.visual.color
    const cssColor = rgba.css(color) === 'rgba(0, 0, 0, 0)' ? 'black' : rgba.css(color)
    this.fillColor = _.get(props, 'filled') ? cssColor : 'rgba(0, 0, 0, 0)'
  }
  if (_.has(props, 'stroked')) {
    const color = layer.visual.strokeColor
    const cssColor = rgba.css(color) === 'rgba(0, 0, 0, 0)' ? 'black' : rgba.css(color)
    this.lineColor = _.get(props, 'stroked') ? cssColor : 'rgba(0, 0, 0, 0)'
  }
...
MapboxVectorTileCatalogItem.prototype.updateLayerChannel = function(layer, props, channel, scaleKey) {
  ...
  const that = this
  const styleFunc = function(uniqueId) {
    return {
      fillStyle: that.fillColor,
      strokeStyle: that.lineColor,
  ...
  if (this.imageryLayer.imageryProvider) {
    this.imageryLayer.imageryProvider._styleFunc = this._styleFunc = styleFunc
  }

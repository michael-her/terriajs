...
var FrameRateMonitor = require("terriajs-cesium/Source/Scene/FrameRateMonitor")
  .default;
var Globe = require("terriajs-cesium/Source/Scene/Globe").default;
var ShadowMode = require("terriajs-cesium/Source/Scene/ShadowMode").default
 ...
 * @param {Integer} [options.minimumZoomDistance] The minimum magnitude, in meters, of the camera position when zooming. Cesium defaults to 1.0.
 */
var TerriaViewer = function(terria, options) {
  ...
  this._maximumLeafletZoomLevel = options.maximumLeafletZoomLevel;
  // michael
  this._minimumZoomDistance = options.minimumZoomDistance
  this._hideAllCredit = options.hideAllCredit
...
9TXL0Y4OHwAAAABJRU5ErkJggg==";
  ...
  globe.shadows = ShadowMode.RECEIVE_ONLY;
...
us via email at " +
  ...
  // michael
  // View in east-north-up frame, Cartesian3.UNIT_Z = (0.0, 0.0, 1.0)
  // scene.camera.constrainedAxis = Cartesian3.UNIT_Z
  
  // constraints camera height by meter
  if (defined(this._minimumZoomDistance)) {
    scene.screenSpaceCameraController.minimumZoomDistance = this._minimumZoomDistance;
  }
  // michael

  if (!this._hideAllCredit) {
...
TerriaViewer.prototype.selectLeaflet = function() {
  ...
  map = L.map(this._mapContainer, {
    ...
    maxZoom: this._maximumLeafletZoomLevel,
      ...
      if (!defined(that.checkedStartupPerformance)) {
        ...
        that.monitor.lowFrameRate.addEventListener(function() {
          if (!that.terria.cesium.stoppedRendering) {
            that.terria.error.raiseEvent({
              title: i18next.t('alert.slowPerformanceTitle', "Unusually Slow Performance Detected"),
              message: i18next.t('alert.slowPerformanceMessage',

...
import {getAttributeAccessor} from '../Layers/base-layer'
...
var MAX_TILE_REQUEST = 64;
var MAX_DOMAIN_ENTITY = 30000;
...
var GeowaveVectorTileImageryProvider = function(options) {
  ...
  this._originalUrls = options.urls
  ...
  // reverse y 타일 스키마를 지원하기 위해 URL을 조작함
  this._uriTemplates = options.urls.map(url => new URITemplate(url.replace("{-y}", "{y}")))
  ...
  this._updateLayerDomain = options.updateLayerDomain;
...
Object.defineProperties(GeowaveVectorTileImageryProvider.prototype, {
  url: {
    get: function() {
      const index = _.random(this._originalUrls.length - 1)
      return this._uriTemplates[index].expression;
    }
  },
...
GeowaveVectorTileImageryProvider.prototype._buildImageUrl = function(
  ...
  const index = _.random(this._originalUrls.length - 1)
  y = this._originalUrls[index].indexOf("{-y}") !== -1 ? Math.pow(2, level) - y - 1 : y;
  return this._uriTemplates[index].expand({
    z: level,
    x,
    y,
...
GeowaveVectorTileImageryProvider.prototype._requestImage = function(
  ...
  // CACHE
  // michael
  if (this._tileCache.promise[path]) {
    return this._tileCache.promise[path]
  } else { // not requested
    // Console.log(`[provider._requestImage] ${this._tileCache.order.length}/${MAX_TILE_CACHE}`)
    // CANCEL EXCEEDED REQUEST
    if (this._tileCache.order.length >= MAX_TILE_REQUEST) {
      let order

      if (nativeTile.level === this._tileCache.order[this._tileCache.order.length - 1].tile.level) {
        // BY SCREEN SPACE CULLING
        for (let i = 0; i < this._tileCache.order.length; ++i) {
          const nt = this._tileCache.order[i].tile
          if (nt.level !== nativeTile.level) continue

          const r = this.tilingScheme.tileXYToRectangle(nt.x, nt.y, nt.level)
          const t = this.tilingScheme.positionToTileXY(Rectangle.southwest(r), nt.level)
          const dx = Math.abs(t.x - x)
          const dy = Math.abs(t.y - y)
          
          if (dx >= 8 || dy >= 8) {
            order = this._tileCache.order.splice(i, 1)[0]
            // Console.log('[provider._requestImage] abort by culling', order.tile.level)
            break;
          }
        }
      }
      ...
      if (!defined(order)) {
        // BY ZOOM LEVEL ORDER
        // most lowest zoom level tile
        order = _.head(this._tileCache.order.splice(this._tileCache.order.length - 1, 1))
        // Console.log(`[provider._requestImage] abort by level`, order.tile.level/*, this._tileCache.order.map(it => it.tile.level)*/)
      }
      
      if (this._tileCache.request[order.path].cancelFunction) {
        this._tileCache.request[order.path].cancelFunction() // abort pending request
      }
      delete this._tileCache.request[order.path]

      delete this._tileCache.promise[order.path]
    ...
    this._tileCache.order.push({path, tile: nativeTile})
    // MUST sorted
    this._tileCache.order = _.sortBy(this._tileCache.order, compareZoomFn)
    
    const resource = Resource.createIfNeeded(url);
    this._tileCache.request[path] = resource.request
    this._tileCache.promise[path] = resource.fetchArrayBuffer().then(data => {
      // if promise removed from cache, just reject here
      if (!defined(this._tileCache.promise[path])) {
        return
      }
      ...
      this._tileCache.order.splice(_.findIndex(this._tileCache.order, order => order.path === path))
      delete this._tileCache.request[path]
      delete this._tileCache.promise[path]
      return ret
    })
    return this._tileCache.promise[path]
  }
...
GeowaveVectorTileImageryProvider.prototype._drawTile = function(
  ...
  for (let i = 0; i < layer.length; i++) {
    ...
    // 피쳐에 고유키가 없거나 중복이 아닌 경우에만 데이터 도메인에 포함
    if (!defined(uniqueId) || !this._data.index.has(uniqueId)) {
      this._data.index.add(uniqueId)
      rows.push(feature.properties)
    }
    
    const fillColor = fillStyle ? fillStyle(feature.properties) : 'black' // instead of undefined
    const strokeColor = strokeStyle ? strokeStyle(feature.properties) : 'black' // instead of undefined
    ...
    if (feature.type === POLYGON_FEATURE || feature.type === POLYLINE_FEATURE) {
      ...
      const radius = lineWidth ? lineWidth(feature.properties) : style.lineWidth
  ...
  if (rows.length) {
    let prev = this._data.rows.length
    if (prev + rows.length > MAX_DOMAIN_ENTITY) {
      let removed = this._data.rows.splice(0, (prev + rows.length) / 3)
      removed = Array.isArray(removed) ? removed : [removed]
      prev = this._data.rows.length
      this._data.index.size > 0 && removed.forEach(row => this._data.index.delete(row[this._uniqueIdProp]))
    }
    this._data.rows = _.concat(this._data.rows, rows)
    this._styleCache.table = new KeplerTable({data: this._data, metadata: {}})
    this._updateLayerDomain && this._updateLayerDomain()
...
GeowaveVectorTileImageryProvider.prototype.createHighlightImageryProvider = function(
  ...
  var imageryProvider = new GeowaveVectorTileImageryProvider({
    urls: this._uriTemplates.map(template => template.expression),

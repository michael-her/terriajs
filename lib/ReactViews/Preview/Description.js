...
const layerMetaDataSchema = {
  ...
  "layer": {
    ...
    "spatialInfo": {
      ...
      "spatialType": false,
      "spatialTypeInt": false
    }
  }
}

const columnMeteDataSchema = [{
  "name": true,
  "maxOccurs": false,
  "minOccurs": false,
  "nillable": true,
...
const Description = createReactClass({
  ...
  render() {
    // Console.log('[Description.render]', {meta: this.props.item.metadata})
    const { t } = this.props;
    ...
    return (
      <div className={Styles.description}>
        ...
        >
          {/* michael: gwMvt descriptions */}
          <If condition={defined(catalogItem.metadata.viewingLayerMetaData)}>
            <div className={Styles.metadata}>
              ...
              >
                <MetadataTable
                  metadataItem={catalogItem.metadata.viewingLayerMetaData}
                  // metaDataSchema={layerMetaDataSchema}
                />
              </Collapsible>
            </div>
          </If>
          <If condition={defined(catalogItem.metadata.columnMetadata)}>
            <div className={Styles.metadata}>
              <Collapsible
                title={t("description.columnsDetails")}
                isInverse={true}
                // startsOpen
              >
                <MetadataTable
                  metadataItem={catalogItem.metadata.columnMetadata}
                  metaDataSchema={columnMeteDataSchema}
                />
              </Collapsible>
            </div>
          </If>
          ...
          <If
            condition={
              // michael: bug fix?
              defined(catalogItem.metadata.serviceMetadata) &&
              catalogItem.metadata.serviceMetadata.items.length > 0
            }
          >
            <div className={Styles.metadata}>
              <Collapsible
        ...
        <If condition={!catalogItem.hideSource}>
          ...
          {/*          })}*/}
          {/*        <br />*/}
          {/*        <Link key="wfs_wcs" url={catalogItem.dataUrl} text={catalogItem.dataUrl} />*/}
          {/*      </When>*/}
          {/*      <When*/}
          {/*        condition={*/}
